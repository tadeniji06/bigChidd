  ...
  useEffect(() => {
    const fetchPredictions = async () => {
      try {
        ...
        `);
        if (data?.matches) {
          // Transform into your UI’s structure
          const formatted = data.matches.map((match: any, index: number) => ({
            _id: `pred_${index}`,
            predictedWinner: "draw",
            odds: (2.7 + Math.random() * 0.7).toFixed(2), // random odds until you add real ones
  ...
  if (loading)
    return (
      <div className="bg-black min-h-screen flex items-center justify-center">
        <p className="text-green-400 animate-pulse">Loading predictions...</p>
      </div>
    );
        ...
        {predictions.map((prediction) => (
          ...
          >
            ...
            <p className="text-xs text-gray-400 text-center">
              {new Date(prediction.match.utcDate).toLocaleString("en-GB", {
                weekday: "short",
                day: "2-digit",
                month: "short",
                hour: "2-digit",
